# MODIFICATION to first run code: 
# 1.  Shove probability mass toward more unequal distributions by addition of warping functions which take from other groups and give to group 1
# 2.  Power shocks larger: on uniform from .05 to .9 now, up from .05 to .6
# 3.  Power decay broader range: now on uniform from 0 to .5
# 4.  Observations of trust and commitment now noiser.  From .01 to .8 instead of .2.
# 5.  Out of paranoia for overwriting and lack of desire to mess with filenames, add random number to output csvs.
# 6.  Make the writing of filenames into an output list file actually work in embarassingly parallel mode by appending.

# SECOND MODIFICATION: all previous runs failed/produced bad results.  This code actually works though.  So I'm going to keep all the noise 
# things, i.e., modifications 2:6 above.  They'll all hash out in sheer number of runs, and this goes fast, so it's cool.  
# then I select 25% of the runs to be subject to each extreme inequality and apply #1. 
# so MODIFY CHAPTER to describe the new parameter ranges as described here. 

# note: office computer R, which this is verified on, is 3.1.2. 
...
# this might bugger up a little with floating point math, but not enough to change results.
