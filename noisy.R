...
# 3.  Out of paranoia for overwriting and lack of desire to mess with filenames, add random number to output csvs.
# 4.  Make the writing of filenames into an output list file actually work in embarassingly parallel mode by appending.
...
simulate.ch9 <- function(runs) {
  ...
  for (j in 1:by1k) {
    ...
    noisename <- runif(1, 0, 1) * results[1000,5]
    k <- j + noisename
